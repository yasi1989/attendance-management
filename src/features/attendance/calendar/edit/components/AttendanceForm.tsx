...
import { useAttendance } from '@/features/attendance/calendar/edit/hooks/useAttendance';
...
import { HALF_DAY_TYPES, ATTENDANCE_TYPES } from '@/features/attendance/calendar/edit/const/attendanceConst';
import CalendarTimeField from './CalendarTimeField';
import InputDateFormField from '@/components/InputDateFormField';
...
const AttendanceTab = ({ dateString }: AttendanceTabProps) => {
  const { form, onSubmit, resetAttendanceForm, resetHalfDayForm, attendanceType, isHalfDay } =
    useAttendance(dateString);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-6">
            <InputDateFormField form={form} name="date" label="勤務日" disabled />
            ...
            {attendanceType === 'PAID_LEAVE' && (
              <InputCheckboxFormField form={form} name="isHalfDay" label="半休申請" onValueChange={resetHalfDayForm} />
            )}
            {isHalfDay && (
