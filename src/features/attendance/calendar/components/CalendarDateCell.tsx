...
const CalendarDateCell = ({ day, currentDate, attendanceData, holidayInfo }: CalendarDateCellProps) => {
  ...
  return (
    <Link href={`/attendance/calendar/edit/${formatDateToISOString(day)}`} className="block">
      <div
        className={cn(
      ...
      >
        ...
        >
          ...
          {holidayInfo && isCurrentMonth && (
            <div
              className={cn(
                'text-xs bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 px-1.5 py-0.5 rounded font-medium',
              )}
            >
              {holidayInfo.name}
            </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              {attendanceData.overtimeHours && attendanceData.overtimeHours > 0 && (
                <div className="text-xs bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 px-1.5 py-0.5 rounded font-medium">
                  残業
                </div>
              )}
            </div>
