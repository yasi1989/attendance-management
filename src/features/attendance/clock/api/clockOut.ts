...
export const clockOut = async (): Promise<ClockOutResult> => {
  try {
    ...
    if (!contextResult.success) return { success: false, error: contextResult.error.message };
    ...
    await db.update(attendances).set({ endTime }).where(eq(attendances.id, existing.id));
