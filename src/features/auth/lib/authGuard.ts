...
export const requireRole = async (allowsRoles: RoleCodeType[]): Promise<Result<{ user: PublicUserWithRole }>> => {
...
export const checkHasCompany = (user: PublicUserWithRole): Result<PublicUserWithCompany> => {
  if (!user.companyId) {
